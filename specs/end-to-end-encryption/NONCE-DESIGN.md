...
**A browser presents the same ticket in `Sec-WebSocket-Protocol`** (added 2026-09-14), because a browser `WebSocket` cannot set `X-TB-Ticket` but can offer subprotocols — which are still a header, so the ticket still never touches a URL. The client offers exactly `threadbase-e2ee-v1, tb-ticket.<ticket>`. The server consumes the ticket with the header path's semantics — the same synchronous consume, single use, 30 s, no fallthrough to a bearer or `?key=` — and the 101 selects `threadbase-e2ee-v1` and nothing else (`handleProtocols` is pinned in `mountWebSocket`; `ws` would otherwise echo the first offer, which could be the ticket). Offering `threadbase-e2ee-v1` without a ticket is a sealed attempt and is refused `401`. A ticket in **both** the header and a subprotocol (or two ticket offers) is refused `401` rather than resolved by preference, and every presented ticket is spent and its context destroyed. An offer no socket can follow — one `ws` would reject (a duplicate, a non-token), or a `tb-ticket.` offer without `threadbase-e2ee-v1` — is answered `400` before any ticket is consumed. `GET /api/info` advertises support as `e2ee.wsTicketSubprotocol: true`; a browser client must refuse encrypted pairing against a server without it rather than fall back to plaintext.
