...
**The trigger has now fired, re-checked at the point this section said to re-check it.** It had not fired when the Noise client merged in #758: outside tests, nothing imported `services/e2ee/*`, so the module was unreachable from Metro's entry graph. The pairing wiring changed that. On tb-mobile `main` as of 2026-08-20, eleven non-test files import `services/e2ee/*` — including `app/settings.tsx`, `stores/servers.ts` and `services/pair-exchange.ts` — so `@stablelib`'s X25519, ChaCha20-Poly1305, SHA-256 and HMAC ship in the bundle, from build 204 onward. `ITSAppUsesNonExemptEncryption` was corrected from `false` to `true` in tb-mobile at that point.
...
**The lead time is real, and it is ANSSI's *then* Apple's — two waits in series, not one.** [Apple's overview](https://developer.apple.com/help/app-store-connect/manage-app-information/overview-of-export-compliance) gives the sequence as: determine requirements, submit documentation, **await approval**, then attach the approved documentation to a beta build or an app version build. Apple reviews the upload before any build can carry it, and that gate covers **TestFlight as well as App Review** — TestFlight is the earlier of the two, so it is the one that binds.
...
- ~~**Whether ANSSI's acknowledgement is required before uploading to Apple.**~~ **Resolved 2026-08-20, and against the write-ups.** App Store Connect's own questionnaire was run to the end: answering "standard algorithms in addition to the OS" and "available in France" produces *"you need to upload a copy of the following: **French encryption declaration approval form**"* — the approval received back, not the declaration sent. So ANSSI's response is a prerequisite for the Apple upload, and the completed form does not suffice. The several write-ups saying otherwise trace to a 2016 source; the questionnaire is Apple's current text and outranks them. This is why the lead time above is two waits rather than one.
