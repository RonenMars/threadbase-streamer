...
**`publicUrl` is `string | null`, and `null` is the ordinary case.** It is what the server believes its public address to be, which is nothing at all unless an operator set `--public-url`, `THREADBASE_PUBLIC_URL`, or `public_url:` in server.yaml. `src/server.ts` declares `private publicUrl: string | null = null`, and that value is passed into msg2 unchanged. A streamer paired over the LAN therefore sends `"publicUrl": null`, so a client that requires a string here refuses every pairing it is most likely to meet. Note that `publicUrl` follows the *server's* configuration rather than the address a client dials: a streamer with `public_url` set sends that string even when the client pairs against its LAN address, so dialling the LAN IP is not by itself a way to exercise the `null` branch. The first hardware pairing run was made against a streamer that had `public_url` configured, so it produced a string and did not cover `null` on a device — [streamer#667](https://github.com/RonenMars/threadbase-streamer/issues/667) tracks closing that gap. Both sides are unit-covered either way. The key is **always present**: the payload is a typed object literal and `JSON.stringify` drops only `undefined`, never `null`. A client must accept `null` and reject the key being absent — the two are different failures, and collapsing them loses the one that means the server is not speaking this contract. `machineName` is a plain string in every case; `os.hostname()` has no null result.
