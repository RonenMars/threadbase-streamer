...
**Resolved 2026-09-04 — the no-env-var rule is retired as unenforceable, not patched.** The `--no-e2ee` flag itself stays exactly as §6.4 describes it: a `serve` option, sugar for `--feature e2ee=false`, with no key or variable of its own. What is retired is the guarantee this entry was written to provide — "no environment variable may hold encryption off" — because `THREADBASE_FEATURE_E2EE` exists by construction of the registry and the only ways to remove it were an `e2ee`-only exception inside `resolveFeatureFlags()` (which breaks the uniform five-rung resolution every other flag relies on and the documented stage-1 enable path for supervised instances) or pretending the variable is not there. Neither was taken. `src/feature-flags.ts` is untouched; `e2ee` keeps resolving through `override → env → cli → yaml → default` like every other flag.

Why retiring it is safe rests on a fact about the client, verified against `tb-mobile` `origin/main`: both channels decide sealed-vs-plaintext from client-held state only, never from what the server currently claims. `services/authed-fetch.ts` routes through `sealedFetch` whenever `isPinned(target)` — `requireEncryption === true && !!serverPublicKey && !!id` — and `services/ws-client.ts` gates on the same two fields. The only code path anywhere in the app that sets `requireEncryption` back to `false` is a deliberate user tap inside a destructive-styled confirm alert (`components/servers/ServerEncryptionSection.tsx`). So a server that goes plaintext via this variable, a `server.yaml` line, or any other source does **not** silently downgrade its pinned devices — it breaks them loudly on every request, which is §6.3 working as written. The residual harms are narrower than this entry assumed: first-contact pairing (a device that was never pinned gets no protection to lose) and operator self-deception (a box believed to be encrypting production traffic that is not). Both are real; neither is the "previously-protected device silently downgraded" harm the rule guarded, which would need the client to trust server-side state over its own pin, and it does not. That makes this a mechanism rule guarding a harm the mechanism largely cannot cause.

The replacement rule is consequence-based and source-agnostic: **a boot that serves plaintext announces itself**, naming the deciding source and the live pinned-device count, whichever rung disabled the flag (`override`, `env`, `cli`, `yaml`); only the registry `default` is exempt, because off-by-default before stage 2 is not a decision anyone made this boot. `e2ee.disabled` at warn carries `reason: <source>`, and `/api/info`'s `reason` names the actual rung instead of collapsing `env`/`yaml`/`default` into one generic line. The warning fires regardless of the count, including at zero: for `env` and `yaml` there is no typed flag, so the boot line is the operator's only confirmation the "off" took effect, and a warning that only speaks once a device has already paired against plaintext is a lagging indicator where a leading one is wanted.

A boot *refusal* gated on the pinned count was considered and dropped. It would have made that count load-bearing, and the count deliberately includes rows whose pairing never completed — a lost `msg2` leaves a row that is pinned but has never connected (#744), so it counts intent to encrypt, not devices in anyone's hand. Under a refusal, one such ghost row could lock an operator out of disabling encryption for a device that does not exist. Under this resolution the count stays advisory, the regime #744's caveat was written for, so #744 is not a prerequisite.

The `env`-beats-`cli` precedence is unchanged and stays pinned by `__tests__/e2ee-no-e2ee-flag.test.ts`.
