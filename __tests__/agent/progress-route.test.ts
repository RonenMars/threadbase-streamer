...
describe("progress route", () => {
  ...
  // ── M4: X-Progress-Timestamp is REQUIRED (no replay via omitted header) ──────
  ...
  it("rejects a valid signature with NO timestamp header (401, no broadcast)", async () => {
    const deps = makeDeps();
    const app = makeApp(deps);
    const body = event({ eventId: "evt_no_ts" });
    const raw = JSON.stringify(body);
    const res = await app.request(`/internal/sessions/${body.sessionId}/progress`, {
      method: "POST",
      headers: {
        "content-type": "application/json",
        "x-progress-signature": sign(raw, SECRET),
        // deliberately omit x-progress-timestamp
      },
      body: raw,
    });
    expect(res.status).toBe(401);
  ...
  it("rejects an unparseable timestamp header (401, no broadcast)", async () => {
    const deps = makeDeps();
    const app = makeApp(deps);
    const body = event({ eventId: "evt_bad_ts" });
    const raw = JSON.stringify(body);
    const res = await app.request(`/internal/sessions/${body.sessionId}/progress`, {
      method: "POST",
      headers: {
        "content-type": "application/json",
        "x-progress-signature": sign(raw, SECRET),
        "x-progress-timestamp": "not-a-number",
      },
      body: raw,
    });
    expect(res.status).toBe(401);
  ...
  it("rejects a stale timestamp beyond the skew window (401, no broadcast)", async () => {
    const deps = makeDeps();
    const app = makeApp(deps);
    const body = event({ eventId: "evt_stale_ts" });
    const raw = JSON.stringify(body);
    const stale = String(Math.floor(Date.now() / 1000) - 10_000); // skew is 300s
    const res = await app.request(`/internal/sessions/${body.sessionId}/progress`, {
      method: "POST",
      headers: {
        "content-type": "application/json",
        "x-progress-signature": sign(raw, SECRET),
        "x-progress-timestamp": stale,
      },
      body: raw,
    });
    expect(res.status).toBe(401);
