...
### `npm warn Unknown env config "global-ignore-file"` on every deploy step
...
Setting up a Windows machine for the first time? [docs/guides/windows-setup.md](guides/windows-setup.md) walks the setup order (Node version → `npm install` → stale `node_modules` → worktree gotchas → deploying) and links back into the entries below.

### `deploy.ps1` crashes with `fatal: not a git repository: (NULL)` / `You cannot call a method on a null-valued expression` *(Windows)*

**When:** Running `pwsh scripts/deploy.ps1 deploy` (with or without `-Force`, before the fix below) against a checkout whose `.git` file points at a worktree gitdir that doesn't exist on this machine — e.g. a directory copied or synced from another machine's `git worktree`, where `.git` still reads `gitdir: /Users/<name>/.../worktrees/<name>` (a macOS/Linux path) or an equivalent path from a different Windows user profile. `git rev-parse --abbrev-ref HEAD` (in `Invoke-PredeployCheck`) or `git rev-parse --short HEAD` (in the release-stamping step) prints `fatal: not a git repository: (NULL)` to stderr and nothing to stdout; PowerShell then calls `.Trim()` on the `$null` result and dies with an unrelated-looking `You cannot call a method on a null-valued expression`.
**Cause:** `git worktree add` records the *absolute path* to the gitdir inside `.git` at creation time. Copying or syncing the worktree directory to another machine (or another path on the same machine) does not update that path, so every `git` command run inside it fails immediately — this predates and is unrelated to whatever `-Force` was supposed to bypass. Both `Invoke-PredeployCheck` (branch/dirty check) and the release-SHA lookup in `Invoke-Deploy` called `git` unconditionally with no fallback.
**Fix (real):** re-point the worktree or re-clone. From the *actual* repo (the one with a working `.git` directory, not a `.git` file), either fix the recorded path (`git worktree repair <path-to-broken-worktree>` if the original gitdir still exists) or drop the stale entry (`git worktree remove --force <path>` / `git worktree prune`) and create a fresh one with `git worktree add`.
**Fix (workaround, already applied):** `scripts/deploy.ps1`'s `Invoke-PredeployCheck` and the release-SHA lookup now check `$LASTEXITCODE` after each `git` call. If git is unusable, `-Force` proceeds with a warning and falls back to a `nogit-<UTC-timestamp>` release tag (e.g. `cli.nogit-20260820001751.cjs`) instead of a commit SHA; without `-Force` it fails fast with a clear message instead of the null-reference crash. This does not fix the underlying broken worktree — it only lets an otherwise-working checkout deploy despite it. See [docs/guides/windows-setup.md](guides/windows-setup.md#broken-git-worktree-link) for how this arises and how to actually fix the link.

---

### Windows deploy completes but the server crashes on every start with a stale `better-sqlite3` binary (silent even though `npm install`/`deploy.ps1` reported success)

**When:** `deploy.ps1` runs to completion (or even reports a healthy deploy once), then a later deploy — after a Node version change, a `node_modules` reinstall, or switching between a compiled-from-source and a prebuilt `better-sqlite3` — leaves the server crashing on every start, or `cache.warmup_failed` / `Could not locate the bindings file` / `NODE_MODULE_VERSION` mismatch errors, even though the deploy script itself printed no error.
**Cause:** the deploy step that copies `better-sqlite3` (and `bindings`, `file-uri-to-path`) from the repo's `node_modules` into `~/.threadbase/node_modules` used `Copy-Item -Recurse` straight into an existing destination directory. `Copy-Item -Recurse` **merges** into an existing directory rather than replacing it — so a previously-copied `build/Release/*.node` compiled for an old Node ABI (or a stale `prebuilds/` tree) survives alongside the newly-copied files and can still be the one Node resolves and loads. The equivalent copy step for the `migrations`/`runtime-migrations`/`pg-migrations` trees already wiped the destination first for the same reason; the `better-sqlite3` copy loop didn't.
**Fix (applied):** `scripts/deploy.ps1`'s `better-sqlite3` copy loop now does `Remove-Item -Recurse -Force` on the destination before `Copy-Item -Recurse`, matching the migrations copy. If you hit this on a checkout that predates the fix, delete `~/.threadbase/node_modules/better-sqlite3` (and `bindings`, `file-uri-to-path`) by hand and redeploy.
**A `Remove-Item`-before-copy fix needs the old process stopped first:** Windows locks a loaded native addon (`*.node`) against deletion/overwrite by the process that `dlopen`'d it, so wiping the destination while the previous server is still running fails with `Access to the path '...\better-sqlite3\prebuilds\...' is denied` — the exact opposite problem from the stale-binary bug above, but caused by the fix for it. `deploy.ps1` now stops the scheduled task and kills any stale port-8766 listener (`Invoke-StopServiceForCopy`) *before* the native-module copy step, not just afterward in `Invoke-Kickstart`. If you see this `Access ... is denied` error on a checkout that predates this second fix, stop the running instance by hand (`Stop-ScheduledTask -TaskName Threadbase` + kill whatever still holds port 8766) before redeploying.
**Related:** this is a different failure mode from the *nested* `@threadbase-sh/scanner` `better-sqlite3` binding never being built in the first place — see [that entry](#nested-threadbase-shscanner-better-sqlite3-binding-never-built-windows-dev-checkouts) below. That one is "the binding was never compiled"; this one is "an old binding was compiled once and never got replaced."
