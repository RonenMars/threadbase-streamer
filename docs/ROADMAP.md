...
The redirection is unverified on a real Windows box — it is covered only by content assertions in `__tests__/deploy-windows-script.test.ts` and `__tests__/lifecycle/prod-commands.test.ts`, which now do run on Windows.

That last clause was false when written on 2026-08-10, and the correction is the point.
`Smoke (windows-latest)` was cited here as covering this, but at the time that job ran a hand-curated allowlist of eight files and neither of those files was on it — so the assertions ran on Linux only and nothing about them was Windows-verified.
...
The PowerShell follow loop in `cli/prod.ts` is not exercised at all: `Get-Content` and file-share semantics only appear at runtime on Windows, so any change there still needs a manual check on a real box.
