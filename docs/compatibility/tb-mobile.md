...
The flag defaults off (`THREADBASE_FEATURE_PTY_HOST=1` enables it), no endpoint, response field, status, or event name changes, and a machine reboot still follows the existing registry-rehydration path because the host is gone too.
