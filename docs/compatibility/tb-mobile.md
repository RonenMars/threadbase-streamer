...
`limit`, `offset`, `sort`, `project`, `provider`, `refresh`, `include`, `isImportedFromClaude`, `isImportedFromCodex`, `isImportedFromCursor`, `msg_limit`, `before_index`, `anchor_index`, `after_index`, `q`, `path`

`provider` on `GET /api/conversations` and `GET /api/conversations/count` is `"claude-code"` | `"codex-cli"` | `"cursor"`; `"cursor-cli"` is accepted and treated as `"cursor"`. `include` is `"all"` | `"conversations"` | `"subagents"` (omit = hide subagents unless the `subagentSessions` flag is on). Import flags are `1`/`true` or `0`/`false`; omit = no filter. Examples: `?provider=cursor&isImportedFromClaude=0`, `?provider=cursor&isImportedFromCodex=1` on `/count`, `?provider=cursor&include=all`, `?provider=cursor&include=subagents`.
...
- Session — new optional field (Codex live-session support): `provider` (`"claude-code"` | `"codex-cli"` | `"cursor"`; absent/undefined means `claude-code` for older data). `conversationId` always equals `id` for the lifetime of a live session, regardless of provider — it is never rekeyed once a client has navigated to it.
- Session — new optional field (Codex / Cursor live-session support): `boundConversationId`. Set once a *fresh* live session's underlying persisted conversation file is discovered after the fact (Codex rollout id, Cursor `agent-transcripts` run id — both CLIs assign their own id that isn't known until the file appears). Distinct from both `conversationId` (always `=== id`, the stable mobile deep-link alias) and `resumedFromConversationId` (set only on the resume flow) — older clients that don't know this field can safely ignore it.
...
- Session — new field `promptSuggestion`: Claude Code's predicted next prompt (the dim text in its input line, e.g. `add type hints and a docstring`), or `null`. Always serialised, never omitted, for the same merge-latch reason as `subStatus`. Set only while the session is `waiting_input`; cleared on any other status.
It is read from the PTY's cell attributes, not the text: the suggestion is painted as SGR 2 (faint) and renders identically to typed input, so the text scrape cannot tell them apart. Claude only — Codex and Cursor dim placeholder text that is not a prediction.
- WebSocket — new event `prompt_suggestion`: `{ type, sessionId, text: string | null, updatedAt }`, scoped to that session's subscribers, same minimal-frame contract as `session_phase`; `text: null` means cleared. Additive; the session field is the source of truth for GET and reconnect.
Adopting it (a tap-to-send chip, long-press to fill the composer) is a tb-mobile change.
- pty-host: adds a `prompt-suggestion-change` event **without** bumping the protocol version. A bump makes `connect()` shut down a surviving host and its sessions, and an older streamer's event switch ignores an unknown event, so the addition is safe both ways.
...
- Conversation list item: `id`, `title`, `projectPath`, `messageCount`, `lastActivity`, `firstMessage`, `lastMessage`, `preview`, `model`, `provider`
- Conversation list item — booleans always present on the list payload (additive for older clients that ignore them): `isImportedFromClaude`, `isImportedFromCodex`, `isImportedFromCursor`. Set when a Cursor `agent-transcripts` file is a copy of another provider; `isImportedFromCursor` stays false until Claude/Codex providers set it. Filter the list/count with the matching query params.
- Conversation detail: `meta` object + `messages` array + `message_pagination` object
- Conversation detail `meta` — new optional fields (additive; older clients ignore them): `provider` (`"claude-code"` | `"codex-cli"` | `"cursor"`; absent means `claude-code`), `resumable` (boolean — false when the conversation's project dir no longer exists, so resume would fail; history is still served), `unavailable_reason` (`"path_missing"` | `"worktree_removed"`, present only when `resumable` is false). The same fields are also added to the resumable session shape (`status: "on_hold"`) returned by `GET /api/sessions/{id}` for conversation ids. **Behavior note (Codex resume support):** `resumable` for a `codex-cli` conversation previously was always forced to `false`; it now reflects the same on-disk project-path availability check as `claude-code` conversations, since Codex resume (`POST /api/sessions/resume`) is implemented and functional.
...
`host_pressure` and `host_pressure_cleared` are **additive** server→client, server-level events (no `sessionId`) for host starvation — old clients ignore unknown types. `GET /api/info` carries `hostPressure: true` when this build samples and pushes them. `host_pressure` is `{ type, level: "elevated" | "critical", reasons: ("memory" | "event_loop" | "load" | "agents")[], liveAgents, updatedAt }` (reasons non-empty, worst-first; `liveAgents` is the PTY-attached live count). `os?: "darwin" | "linux" | "win32"` is additive and optional — present on those three Node platforms, omitted on anything else; old clients ignore it. On Windows, `load` is CPU busy from `os.cpus()[].times` deltas (`process.platform` is `win32` on both 32- and 64-bit Windows); POSIX still uses `loadavg`. It is broadcast on a level change and unicast on WebSocket open while currently warned. `host_pressure_cleared` is `{ type, updatedAt }`. Informational only: the sampler never holds, kills, or refuses sessions. Absent `hostPressure` means an older server that does not sample.
On macOS, memory uses the pressure-aware free percentage from `/usr/bin/memory_pressure -Q`; if that probe fails or returns malformed output, the memory signal is omitted while event-loop and load classification continue.
...
- A browser cannot set `X-TB-Ticket`, so a sealed socket offers subprotocols `threadbase-e2ee-v1, tb-ticket.<ticket>` instead (NONCE-DESIGN §10). The 101 must select `threadbase-e2ee-v1` — a browser fails a socket that offered protocols and got none — and must never echo the ticket. Gated on `GET /api/info` → `e2ee.wsTicketSubprotocol: true`. An accepted sealed upgrade (header ticket or subprotocol) logs `[e2ee.upgrade] encrypted websocket` with `event: e2ee.upgrade` and never includes the ticket.
...
It also accepts an optional `notificationPrefs` (`{ waitingInput, sessionFailed, quietHours? }`, validated, else 400) and keeps the stored value when a later registration sends none; `PATCH /api/push/preferences` `{ token, prefs }` changes them without resetting delivery health (404 `TOKEN_NOT_FOUND` when the token is not registered), and `POST /api/push/test` `{ token }` sends one real push and returns `{ ok, attempted, succeeded, state }`. All additive: a client that sends no preferences receives everything, as before, and `push.preferences` on `/api/info` says whether a server enforces them. Pushes gain `data.kind: "failed"` and `data.kind: "limited"` (a usage limit, previously sent as `permission`), and an iOS `subtitle`; all ignored by builds that route on `sessionId` alone. Mobile's `sessionComplete`, `diffReady` and `showBadge` settings have no server event and are not part of this contract.
...
`GET /api/info` and `GET /api/push/health` carry an additive `push` object describing what the server can actually deliver: `{ liveActivity: boolean, notifications: boolean, liveActivityReason?: string, notificationsReason?: string }`. `liveActivity` is true only when APNs credentials resolved *and* the sender was wired — the same fact the boot log reports as `live_activity.enabled`. `notifications` is true when the "your turn" notifier (`WaitingInputNotifier` over Expo's relay) was wired, which needs only the push token store to have opened — no credential, flag or env var — so it is false only on a server whose SQLite cache failed to open; the two fields are independent. `liveActivityReason` is present only when `liveActivity` is false and names the missing environment variable, never its value; `notificationsReason` is present only when `notifications` is false. The object is absent on servers that predate it, which a client must read as "unknown" rather than "unavailable". Mobile should hide or disable the Live Activity affordance when `push.liveActivity` is false rather than registering tokens nothing will ever send to.
