...
3. Deep-link and paste confirmation gate on hardware: fingerprint + machine name shown; cancel adds nothing; confirm adds. Camera path adds **without** the *blocking* gate — this is the expected out-of-band channel behavior (mobile-design §3.3), not a defect; a defect would be a gate appearing on the camera path, or no gate on deep-link/paste. **Annotated 2026-09-04 (G-2):** this line was written from the spec, not from an executed camera test — D1 only drove deep-link and paste. G-2's hardware run is the first actual camera-path test, and it found that an E2EE (`spk`-bearing) camera scan does show a *separate*, non-blocking, `Done`-only identity card (`PairCameraIdentityCard`, added #783) before continuing the add — distinct from the blocking `PairConfirmGate` this row exercises, and not documented in mobile-design §3.3 until this correction. "No gate" remains correct for the blocking gate specifically; a legacy (no-`spk`) camera scan still shows nothing, matching a deep link with no `spk`.
