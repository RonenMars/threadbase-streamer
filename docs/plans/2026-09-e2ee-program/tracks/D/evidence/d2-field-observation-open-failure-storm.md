...
**2. The tunnel case is the one to worry about, and the code already says so.** `rate-limit.ts` warns that behind a Cloudflare tunnel every request arrives from `127.0.0.1`. On LAN the two devices had separate addresses and separate budgets. Through `<test-tunnel-host>` they would share one, and this single misconfigured device would have consumed the whole fleet's budget — the exact scenario the comment describes, now with a real device producing it rather than an adversary.
