...
cmd_free_stale_port() {
  ...
  [[ -n "$pids" ]] || { debug "cmd_free_stale_port: port $PORT has no listeners"; return 0; }
  debug "cmd_free_stale_port: port $PORT held by pid(s): $(printf '%s ' $pids)"
...
activate_release() {
  local rel_path="$1"
  debug "activate_release: pointing $INSTALL_DIR/cli.js -> $rel_path"
  ...
  if [[ -f "$sidecar" ]]; then
    ...
    debug "activate_release: version.txt <- $(tr -d '[:space:]' < "$INSTALL_DIR/version.txt") (from sidecar)"
  else
