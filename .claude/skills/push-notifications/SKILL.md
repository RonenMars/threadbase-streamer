...
- **Mid-turn prompts** — a permission gate or AskUserQuestion menu pushes once when it opens (`onPrompt`, wired from `onPermissionChange` / `onLiveQuestion`); Claude's turn stays open across them, so the turn-end push cannot cover them.
- **Payload** — `title: "<emoji> projectName"`, a localized `body` from `notificationCopy.ts` (per token `locale`, en/he/ar/ru, English fallback), `sound: "default"` (iOS is silent without it), `priority: "high"`, `threadId`/`collapseId`/`tag` = sessionId, `data: { sessionId, kind, serverId? }` (mobile routes the tap from those). `serverId` is per recipient token: it is the id the app sent as `serverId` in `POST /api/push/register`, stored in `push_tokens.client_server_id` (migration 024) and added by `ExpoPushSender`. A token registered without one — an older app — gets no `serverId`, so the app uses its default server. It is never the streamer's hostname, which the app cannot resolve. Deliberately **no `lastOutput` and no `sessionName`**: raw terminal output and a prompt-derived title are exactly what the privacy policy says notifications exclude — see [docs/guides/waiting-input-push.md](docs/guides/waiting-input-push.md) before adding a field.
- **Preferences** — per token, enforced at send time: `push_tokens.notification_prefs` (migration 026, JSON, `NULL` = everything on, unparsable = everything on), applied per row in `ExpoPushSender.send(message, { event })` so one device's quiet hours never mute another. `waitingInput` gates `turn_done`/`permission`/`question`, `sessionFailed` gates the failure push; quiet hours (phone's IANA `tz`, global window plus optional per-weekday overrides, overnight windows belong to the day they start) **drop** the push. Set via `notificationPrefs` on `POST /api/push/register` or `PATCH /api/push/preferences` — never re-register to change a toggle, it resets `failure_streak`/`revoked_at`. `POST /api/push/test` sends a real push bypassing preferences. Reported as `push.preferences`. Detail: [docs/guides/waiting-input-push.md](docs/guides/waiting-input-push.md).
- **Failed start** — one `failed` push when a session goes idle with a `failureReason` having never reached a prompt (`readySeen`). Fixed text, never the reason (it embeds paths). A Codex usage-limit `failureReason` on a live session is not this; it already arrives as a permission push.
