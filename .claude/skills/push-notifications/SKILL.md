...
- **Payload** — `title: projectName`, `body: "Waiting for your input"`, `data: { sessionId, serverId? }` (mobile routes the tap from those). `serverId` is per recipient token: it is the id the app sent as `serverId` in `POST /api/push/register`, stored in `push_tokens.client_server_id` (migration 024) and added by `ExpoPushSender`. A token registered without one — an older app — gets no `serverId`, so the app uses its default server. It is never the streamer's hostname, which the app cannot resolve. Deliberately **no `lastOutput` and no `sessionName`**: raw terminal output and a prompt-derived title are exactly what the privacy policy says notifications exclude — see [docs/guides/waiting-input-push.md](docs/guides/waiting-input-push.md) before adding a field.
