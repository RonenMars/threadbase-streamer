...
- `codex-pty-runner.ts` — same for Codex sessions. Blocking startup gates (directory trust, hooks review) become question cards over the `permission` WS transport; a "remember for all projects" answer persists to `~/.threadbase/gate-answers.json` (`services/questions/codexGateAnswers.ts`) and auto-answers future gates. Usage-limit / rate-limit screens (and the soft "usage limit reset available" tip after a failed submit) are detected the same way and surfaced as permission cards with `failureReason` on the session so mobile stops spinning in `running`. Readiness = `Ready` status-bar marker (quiet alone never settles boot — `Starting` / MCP boot lines keep the input queue armed). An 8s flat fallback settles only once the screen is idle (not busy, and compose `›`/`>` or Ready is visible; re-arms while MCP is still loading; covers a truncated Ready bar). Mid-session, `running → waiting_input` when the title's braille spinner stops (Codex CLI 0.156.1 has no Working/Ready in its status bar; Working then Ready remains the fallback when no title is seen), held while an approval/gate/picker card is open; if no turn start appears within 2s after submit (no Ready required), recovers as `submit-stale` so grace/hold is not stuck. `cursor-pty-runner.ts` reads its turn off the `ctrl+c to stop` hint the compose box carries while a turn runs; the hint only shows if the `\r` came after Cursor echoed the prompt, so its submit waits for that echo, and Ctrl+U goes in its own write because Cursor drops a read that starts with it (`docs/troubleshooting.md#cursor-prompt-dropped`). Submit waits for PTY quiescence before `\r` (same redraw-race fix as Claude).
- `session-store.ts` — in-memory registry of managed (PTY) + discovered (process) sessions. All session state mutations go through it.
- `conversation-cache.ts` — SQLite cache of conversation metadata, message tails, projects, and cache_metadata; updated incrementally by `ConversationWatcher` (chokidar). Backs `/api/conversations`, `/api/sessions`, and `/project-chats`. Runs SQLite migrations on open (`db/sqlite-migrate.ts` + `db/migrations/*.sql`, tracked in `schema_migrations`). Line parsers are provider-specific (`parseJsonlLine` / `parseCodexJsonlLine` / `parseCursorJsonlLine`). Import flags (`is_imported_from_claude` / `is_imported_from_codex` / `is_imported_from_cursor`, migration 019) are copied from scanner meta onto list items; Cursor fills the first two, `isImportedFromCursor` stays false until Claude/Codex providers set it.
- `services/conversations/conversationWatcher.ts` — chokidar-backed JSONL tail + directory watcher. Emits per-line events (cache + WS broadcast) and per-file dirty events (cache invalidation). **`watchDirectory()` costs one OS watch handle per file under the root, not one per directory** — chokidar recurses, and per-file `change` events are what `poke()`'s tail self-heal and the external-tail attach need. So the process holds roughly one open fd per conversation transcript on the box (measured 2026-08-09: 2131 fds against 2133 files, ~88% of all fds on the process). That is the design working, not a leak: it tracks the corpus on disk, not live sessions, and `unwatch()`/`close()` do release. macOS has room (2.0% of a 122 880 per-process ceiling); **Linux is the tight one**, since these are inotify watches against the per-user `max_user_watches` (as low as 8192, shared with every other watcher). Exhaustion arrives as ENOSPC on the watcher's `error` event, logged as `watcher.limit_exhausted`. **Closing those handles is slow on macOS**: about 10 ms each, synchronously, so `dispose()` over a real `~/.claude/projects` (~3700 entries) blocked the event loop for 39 s and held the port through every restart. The CLI's shutdown therefore calls `server.close({ exiting: true })`, which `detach()`es the watchers and lets process exit release them; the same flag skips waiting on an in-flight scan and its cache writes and leaves `cache.db` unclosed, because that wait held the port for 51 s when a restart landed during the startup scan, and every SQLite write is a synchronous transaction that WAL recovers.
...
- **`waiting_input`**: Claude printed a prompt marker (`CLAUDE_PROMPT_MARKERS = ["╭", "❯"]` in `pty-manager.ts`, plus a fallback timeout) — idling for user input.
  **Mid-conversation the marker is not enough**: Claude keeps its `❯` input box painted for the whole turn, so the marker alone flipped every submit to `waiting_input` ~50 ms later and the "finished" push fired as the agent *started* (#962). Claude's own turn signal is the terminal title: a spinner glyph (`◐◓◑◒`) from ~40 ms after the submit, `✳` when idle. **OSC 9;4 is never emitted into the streamer's PTY** (Claude Code 2.1.280, whatever `TERM_PROGRAM` says or the PTY answers to its terminal probes); it is still parsed for a terminal that gets it. The idle title also appears when a permission gate paints — 3 ms before the gate, in a separate chunk — so an idle signal only ends the turn on the next quiet check (`QUIET_DETECT_MS`), and only if no gate is on screen; a gate holds the turn open, and once it leaves the screen the turn re-enters the start grace. While a turn is open the markers are ignored; a submit opens it optimistically and lapses after `TURN_START_GRACE_MS` (3 s) if no busy signal follows. Under load the busy signal can land after a guess already settled the turn (the grace lapsing onto `❯`, Codex's `submit-stale`); one within `LATE_TURN_START_MS` (30 s) of the submit puts the session back to `running` with `statusSource: "turn-signal"`, so the real end still sends "finished". A CLI that emits no signal keeps marker-only readiness.
  **Only a signalled end sends "finished".** Every runner marks it `statusSource: "turn-signal"`; `WaitingInputNotifier` skips any other source (a marker, `submit-stale`, a boot timer) and holds the push for `TURN_DONE_SETTLE_MS` (2 s), cancelled if the session leaves `waiting_input` or a gate opens. The decision is logged per turn as `push.turn_decision`, and Claude's turn state as `pty.turn`.
...
- An instant non-zero exit (<2 s, no output) gets a diagnosed `failureReason` (missing project dir, or Claude binary not found) and a matching `failureCode` (`project_dir_missing` / `instant_exit`), which is what the failure push reads — the reason embeds a path and never goes in a push. It reaches the client on the settled session the status bus carries, and is mirrored into `SessionStore` so the session reports `lifecycle: "failed"` rather than "completed".
...
- **A session with no transcript is empty, not missing.** Claude writes `<sessionId>.jsonl` only on the first user turn (measured 0.0 s–86.9 s after `pty.ready`, and never at all if the user opens a session and walks away), so `GET /api/conversations/:id` answers **200 with zero messages** rather than 404 while the file is still to come — the 404 was 62% of every conversation 404 in a three-week production log and is what rendered "Messages failed to load" on a session that was working fine. Two disjoint cases reach that 200, and they are not the same test. A session whose `promptCount` is 0 has not spoken yet and qualifies indefinitely, because an opened-and-abandoned session never gets a file at all. A session that *has* prompted qualifies only while the transcript is genuinely in flight: no path bound yet (`findLiveSessionFilePath`) **and** activity inside `TRANSCRIPT_WATCH_DEADLINE_MS`. **The second case exists because `promptCount === 0` alone had a hole exactly one prompt wide** — sending the first prompt is what flips the counter *and* what makes Claude create the file, so the guard was shut for precisely the 0.3 s–1.7 s the file did not exist (4 sessions, 8 of 203 conversation 404s in a 20.5-day log, every one bracketed by `pty.input_write` → 404 → `session.jsonl_wired`; mobile refetches on submit and landed in it every time). Recency is measured from `lastActivityAt ?? startedAt`, never from `startedAt` alone: spawn-to-first-prompt is human think time and ran to 405.7 s in production, so a spawn anchor would leave the hole open for anyone slow to type. The guard is still narrow where it matters — a bound transcript, or a prompted session gone quiet past the deadline, is real data loss and still 404s.
- **The conversation 404 self-heal fires only on proof.** `handleGetConversation` used to call `cache.invalidate(id)` on every miss. For a Codex rollout the cache row is the only rung of `locateJsonlPath` that can name the file — `findJsonlPath` reconstructs Claude's `<uuid>.jsonl` layout and cannot match `rollout-<ts>-<uuid>.jsonl` by construction — so one transient miss deleted the row and every later request 404'd on a conversation still on disk, taking `/api/sessions/:id` (whose only fallback is that same row) with it. It now invalidates only when the row names a file that is gone, and it invalidates `lookupId`, not the raw request id, which for Codex is a placeholder. `locateJsonlPath` also consults the scanner's `getMetadataCache()` — the same map the list endpoint serves — and, because Codex/Cursor index keys are file paths while clients look up the session uuid, `getConversationsBySessionId`. Cursor history is opt-in `cursorRoots` (default `~/.cursor/projects`); Claude's projects walk cannot see those files.
