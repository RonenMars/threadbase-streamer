...
- `codex-pty-runner.ts` — same for Codex sessions. Blocking startup gates (directory trust, hooks review) become question cards over the `permission` WS transport; a "remember for all projects" answer persists to `~/.threadbase/gate-answers.json` (`services/questions/codexGateAnswers.ts`) and auto-answers future gates. Readiness = `Ready` status-bar marker (quiet alone never settles boot — `Starting` / MCP boot lines keep the input queue armed). An 8s flat fallback settles only once the screen is no longer busy (re-arms while MCP is still loading; covers a truncated Ready bar). Mid-session, `running → waiting_input` after Working then Ready. Submit waits for PTY quiescence before `\r` (same redraw-race fix as Claude).
