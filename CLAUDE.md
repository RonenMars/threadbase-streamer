...
| `e2ee` | off | Application-layer encryption between a paired device and this server, independent of TLS. Today it gates **the capability negotiation only** — `GET /api/info` grew an additive `e2ee: { supported, enabled, version, required, reason? }` object, and the handshake it will gate does not exist yet. `supported` is a build constant, deliberately not the flag: it means "this build speaks the envelope", so reporting the flag would let an operator who switches it on advertise a handshake this build cannot perform, and a client would offer to re-pair for encryption and then fail. That is the half-landed break the negotiation exists to remove, which is why it ships ahead of the handshake rather than beside it. A client reads `enabled`, never `supported` alone; absent means an older server and resolves to today's plaintext path. Off by default and negotiated per device forever — tb-mobile is released and cannot be force-updated, so a server that demanded encryption would break every older install the day its streamer updated. Design: [specs/end-to-end-encryption/design.md](specs/end-to-end-encryption/design.md) §6. |
