...
- **`waiting_input`**: Claude printed a prompt marker (`CLAUDE_PROMPT_MARKERS = ["╭", "❯"]` in `pty-manager.ts`, plus a fallback timeout) — idling for user input.
  **Mid-conversation the marker is not enough**: Claude keeps its `❯` input box painted for the whole turn, so the marker alone flipped every submit to `waiting_input` ~50 ms later and the waiting-for-input push fired as the agent *started*. Claude's own turn signal is OSC 9;4 terminal progress — `9;4;3` at turn start, `9;4;0` exactly at turn end, held across a permission gate (verified on Claude Code v2.1.278). While a turn is open the markers are ignored; a submit opens it optimistically because the prompt echo beats `9;4;3` by ~300 ms, and lapses after `TURN_START_GRACE_MS` (3 s) if no `9;4;3` follows. A CLI that never emits 9;4 keeps marker-only readiness. A modal slash command (`/cost`) holds `9;4;3` until dismissed, so it reads `running` until Esc.
