...
import {
  ...
  readBody,
  writeHonoResponse,
} from "./api/handlers/http-helpers";
...
export class StreamerServer {
  ...
  constructor(config: ServerConfig & { apiKey: string }) {
    ...
    this.registryBoot = new SessionRegistryBoot({
      ...
      resumeSession: (opts) => this.resumeSession(opts),
  ...
  private async handleAdopt(sessionId: string, res: ServerResponse): Promise<void> {
    ...
    if (!projectPath) {
      const jsonlPath = this.conversationHandlers.findJsonlPath(convId);
      const jsonlCwd = jsonlPath
        ? await this.conversationHandlers.readCwdFromJsonl(jsonlPath)
        : null;
      if (jsonlCwd) {
