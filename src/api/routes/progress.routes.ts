...
function stageToRole(
	stage: Stage | string | undefined,
): "worker" | "reviewer" | "signoff" {
...
export const createProgressRoutes = (deps: ApiDeps & AgentDeps) => {
	...
	app.post("/sessions/:sessionId/progress", async (c) => {
		...
		if (
			!verifySignature(rawBuf, sigHeader, deps.agentConfig.webhook.hmacSecret)
		) {
		...
		if (event.type === "stage_transition") {
			...
			const reason =
				(event.payload as { reason?: string } | undefined)?.reason ?? "unknown";
			const msg: WSMessage = {
				type: "turn_failure",
				sessionId: event.sessionId,
				turnId: event.turnId,
